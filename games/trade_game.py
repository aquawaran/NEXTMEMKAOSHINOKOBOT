...
class TradeGame:
    ...
    async def _send_graph(self, message: Message, user_id: int, show_result: bool = False, custom_text: str = None):
        """Отправить график"""
        graph_text = self._create_graph_visual(self.active_games[user_id]['graph_data'])
    ...
    async def _reveal_result(self, message: Message, user_id: int):
        ...
        # Отправляем результат с текстовым графиком
        graph_text = self._create_graph_visual(game['graph_data'])
        await message.reply(status_text + "\n\n" + graph_text)
