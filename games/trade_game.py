...
class TradeGame:
    ...
    async def _send_graph(self, message: Message, user_id: int, show_result: bool = False, custom_text: str = None):
        """Отправить график"""
        graph_path = self._create_graph_visual(self.active_games[user_id]['graph_data'])
        
        if custom_text:
            # Используем кастомный текст
            await message.reply_photo(graph_path, caption=custom_text)
        else:
            # Показываем выбор и через 1 секунду результат
            status_text = f"📊 Трейд\n\n"
            status_text += f"💰 Ставка: {self.active_games[user_id]['bet']} MEM\n"
            status_text += f"🤔 Выбор: {self.active_games[user_id]['direction']}\n"
            status_text += f"⏳ Ожидание результата..."
            
            await message.reply_photo(graph_path, caption=status_text)
    ...
    async def _reveal_result(self, message: Message, user_id: int):
        ...
        direction = "вверх" if game['direction'] == "up" else "вниз"
        status_text = f"📊 ТРЕЙД\n\n"
        ...
        graph_path = self._create_graph_visual(game['graph_data'])
        await self._send_graph(message, user_id, show_result=True, custom_text=status_text)
